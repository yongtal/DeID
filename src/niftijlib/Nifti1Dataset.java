...
public class Nifti1Dataset {
	...
	public String 		ds_hdrname;	// file name for header
	...
	public byte		extension[];	// 4 byte array, byte 0 is 0/1 indicating extensions or not
        ...
        public StringBuffer     orient;         // 1 char orient
        public StringBuffer     originator;     // 10 char originator
        public short            origin[];
        public StringBuffer     generated;      // 10 char generated
        public StringBuffer     scannum;        // 10 char scannum
        public StringBuffer     patient_id;     // 10 char patient_id
        public StringBuffer     exp_date;       // 10 char exp_date
        public StringBuffer     exp_time;       // 10 char exp_time
        public StringBuffer     hist_un0;       // 3 char hist_un0  
        public int              views;          // 1 int 
        ...
        public int              smax;
        public int              smin;
        public StringBuffer	hkey_un0;	// 1 char MRI slice ordering
        public short		dim_un0;	// first slice index
	...
	*/
	...
	ecs.close();
        ...
        public void read_Analyze_Header() throws IOException, FileNotFoundException {
		...
		ecs.readFully(bb,0,10);
		data_type_string = new StringBuffer(new String(bb));
                System.out.println("data_ytpe_string: "+ data_type_string);
		...
		hkey_un0 = new StringBuffer();
		hkey_un0.append((char)(ecs.readUnsignedByte()));
		ss = unpackDimInfo((int)hkey_un0.charAt(0));
		freq_dim = ss[0];
		phase_dim = ss[1];
		slice_dim = ss[2];
                System.out.println("freq, phase, slice dim: "+ freq_dim+" "+phase_dim+" "+slice_dim);
		...
		funused3 = ecs.readFloatCorrect();
                
  
                
                //////////////////////////////////////
                cal_max = ecs.readFloatCorrect();
                System.out.println("cal max: "+ cal_max);
                cal_min = ecs.readFloatCorrect();
                
                compressed = ecs.readFloatCorrect();
                verified = ecs.readFloatCorrect();
                glmax = ecs.readIntCorrect();
                glmin = ecs.readIntCorrect();
                //bb = new byte[80];
                descrip = new StringBuffer();
                for (i = 0; i < 80; i++)
                    descrip.append((char)(ecs.readUnsignedByte()));
                
                bb = new byte[24];
                ecs.readFully(bb,0,24);
		aux_file = new StringBuffer(new String(bb));
                orient = new StringBuffer();
		orient.append((char)(ecs.readUnsignedByte()));
                
                originator = new StringBuffer();
                for (i = 0; i < 5; i++) {
                    origin[i] = ecs.readShortCorrect();
                    originator.append((origin[i]));
                }               
                
                bb = new byte [10];
                ecs.readFully(bb, 0, 10);
                generated = new StringBuffer(new String(bb));
                
                bb = new byte [10];
                ecs.readFully(bb, 0, 10);
                scannum = new StringBuffer(new String(bb));
                
                
                //for (i = 0; i < 5; i++)
                //    System.out.println("hh: "+ecs.readShortCorrect());
                
                bb = new byte [10];
                ecs.readFully(bb, 0, 10);
                patient_id = new StringBuffer(new String(bb));
                bb = new byte [10];
                ecs.readFully(bb, 0, 10);
                exp_date = new StringBuffer(new String(bb));
                bb = new byte [10];
                ecs.readFully(bb, 0, 10);
                exp_time = new StringBuffer(new String(bb));
                bb = new byte [3];
                ecs.readFully(bb, 0, 3);
                hist_un0 = new StringBuffer(new String(bb));
                views = ecs.readIntCorrect();
                vols_added = ecs.readIntCorrect();
                start_field = ecs.readIntCorrect();
                field_skip = ecs.readIntCorrect();
                omax = ecs.readIntCorrect();
                omin = ecs.readIntCorrect();
                smax = ecs.readIntCorrect();
                smin = ecs.readIntCorrect();
    ...
    public void copy_Analyze_Header(Nifti1Dataset A) {
	...
	session_error =	A.session_error;
	regular = new StringBuffer(A.regular.toString());
	hkey_un0 = new StringBuffer(A.hkey_un0.toString());
	freq_dim=A.freq_dim; 
	phase_dim=A.phase_dim; 
	slice_dim=A.slice_dim;
	for (i=0; i<8; i++)
	...
	for (i=0; i<4; i++)
        ...
        */
        ...
        orient = new StringBuffer(A.orient.toString());
        originator = new StringBuffer(A.originator.toString()); 
        for (i = 0; i < 5; i++) 
            origin[i] = A.origin[i];
        generated = new StringBuffer(A.generated.toString());      
        scannum = new StringBuffer(A.scannum.toString());        
        patient_id = new StringBuffer(A.patient_id.toString());     
        exp_date = new StringBuffer(A.exp_date.toString());       
        exp_time = new StringBuffer(A.exp_time);      
        hist_un0 = new StringBuffer(A.hist_un0);         
        views = A.views;          
        vols_added = A.vols_added; 
        start_field = A.start_field;
        field_skip = A.field_skip;
        omax = A.omax;
        omin = A.omin;
        smax = A.smax;
        smin = A.smin;
...
public void write_Analyze_Header() throws IOException, FileNotFoundException {
		...
		ecs.writeFloatCorrect(funused3);
		...
		//ecs.writeFloatCorrect(slice_duration);
                ecs.writeFloatCorrect(compressed);
                
		//ecs.writeFloatCorrect(toffset);
		 ecs.writeFloatCorrect(verified);
		...
		ecs.write(setStringSize(aux_file,24),0,24);
                ...
                ecs.writeByte((int) orient.charAt(0));
                
                //ecs.write(setStringSize(originator,10),0,10);
                for (i =0 ; i < 5; i++)
                    ecs.writeShortCorrect(origin[i]);
                
                ecs.write(setStringSize(generated,10),0,10);
                ecs.write(setStringSize(scannum,10),0,10);
                ecs.write(setStringSize(patient_id,10),0,10);
                ecs.write(setStringSize(exp_date,10),0,10);
                ecs.write(setStringSize(exp_time,10),0,10);
                ecs.write(setStringSize(hist_un0,3),0,3);
                ecs.writeIntCorrect(views);
                ecs.writeIntCorrect(vols_added);
                ecs.writeIntCorrect(start_field);
                ecs.writeIntCorrect(field_skip);
                ecs.writeIntCorrect(omax);
                ecs.writeIntCorrect(omin);
                ecs.writeIntCorrect(smax);
                ecs.writeIntCorrect(smin);
